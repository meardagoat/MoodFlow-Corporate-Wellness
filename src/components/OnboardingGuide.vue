<template>
  <div v-if="showGuide" class="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-1 sm:p-4 animate-fadeIn">
    <div class="bg-white rounded-lg sm:rounded-2xl shadow-2xl max-w-4xl w-full max-h-[98vh] sm:max-h-[90vh] overflow-hidden mx-1 sm:mx-4 animate-slideUp">
      <!-- Header -->
      <div class="bg-gradient-to-r from-orange-500 to-purple-500 px-2 sm:px-6 py-2 sm:py-4 text-white">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-1 sm:gap-3">
            <div class="w-6 h-6 sm:w-12 sm:h-12 bg-white/20 rounded-full flex items-center justify-center">
              <span class="text-sm sm:text-2xl">🚀</span>
            </div>
            <div class="min-w-0 flex-1">
              <h2 class="text-sm sm:text-2xl font-bold truncate">Bienvenue sur MoodFlow !</h2>
              <p class="text-orange-100 text-xs sm:text-base truncate">Découvrons ensemble les fonctionnalités</p>
            </div>
          </div>
          <button
            @click="closeGuide"
            class="p-1 sm:p-2 hover:bg-white/20 rounded-full transition flex-shrink-0"
          >
        ...
        <div class="mt-2 sm:mt-4">
          ...
          <div class="w-full bg-white/20 rounded-full h-1 sm:h-2">
            <div 
              class="bg-white h-1 sm:h-2 rounded-full transition-all duration-500 ease-out"
      ...
      <!-- Interactive Content -->
      <div class="p-2 sm:p-6 overflow-y-auto max-h-[calc(98vh-120px)] sm:max-h-[calc(90vh-120px)]">
        <!-- Step 1: Mood Selection Interactive -->
        <div v-if="currentStep === 1" class="space-y-3 sm:space-y-6">
          <div class="text-center">
            <h3 class="text-lg sm:text-2xl font-bold text-gray-900 mb-1 sm:mb-2">Comment vous sentez-vous ?</h3>
            <p class="text-gray-600 text-xs sm:text-base">Cliquez sur l'humeur qui vous correspond le mieux</p>
          </div>
          
          <div class="grid grid-cols-3 sm:grid-cols-5 gap-1 sm:gap-4">
            <button
              v-for="(mood, index) in interactiveMoods"
              :key="mood.value"
              @click="selectMood(mood.value)"
              :class="[
                'p-2 sm:p-6 rounded-lg sm:rounded-2xl transition-all duration-300 transform hover:scale-105',
                selectedMood === mood.value
                  ? 'bg-gradient-to-br from-orange-500 to-purple-500 text-white shadow-xl scale-105'
                  : 'bg-gray-100 hover:bg-gray-200'
              ]"
            >
              <div class="text-xl sm:text-4xl mb-1 sm:mb-2">{{ mood.emoji }}</div>
              <div class="text-xs sm:text-sm font-semibold">{{ mood.label }}</div>
            </button>
          </div>
          
          <div v-if="selectedMood" class="text-center">
            <p class="text-xs sm:text-lg text-gray-700">Parfait ! Vous avez choisi : <span class="font-bold">{{ getMoodLabel(selectedMood) }}</span></p>
          </div>
        </div>

        <!-- Step 2: Tag Selection Interactive -->
        <div v-if="currentStep === 2" class="space-y-3 sm:space-y-6">
          <div class="text-center">
            <h3 class="text-lg sm:text-2xl font-bold text-gray-900 mb-1 sm:mb-2">Qu'est-ce qui influence votre humeur ?</h3>
            <p class="text-gray-600 text-xs sm:text-base">Sélectionnez les éléments qui vous concernent (optionnel)</p>
          </div>
          
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-1.5 sm:gap-3">
            <button
              v-for="tag in interactiveTags"
              :key="tag.id"
              @click="toggleTag(tag.id)"
              :class="[
                'p-2 sm:p-4 rounded-md sm:rounded-xl transition-all duration-300 transform hover:scale-105',
                selectedTags.includes(tag.id)
                  ? 'bg-gradient-to-r from-orange-500 to-purple-500 text-white shadow-lg'
                  : 'bg-gray-100 hover:bg-gray-200 text-gray-700'
              ]"
            >
              <div class="text-lg sm:text-2xl mb-1 sm:mb-2">{{ tag.emoji }}</div>
              <div class="text-xs sm:text-sm font-semibold">{{ tag.label }}</div>
            </button>
          </div>
          
          <div v-if="selectedTags.length > 0" class="text-center">
            <p class="text-xs sm:text-lg text-gray-700">Excellent ! Vous avez sélectionné {{ selectedTags.length }} élément(s)</p>
          </div>
        </div>

        <!-- Step 3: Anonymous Toggle Interactive -->
        <div v-if="currentStep === 3" class="space-y-3 sm:space-y-6">
          <div class="text-center">
            <h3 class="text-lg sm:text-2xl font-bold text-gray-900 mb-1 sm:mb-2">Confidentialité</h3>
            <p class="text-gray-600 text-xs sm:text-base">Choisissez comment vous souhaitez partager</p>
          </div>
          
          <div class="max-w-md mx-auto">
            <div class="bg-gray-50 rounded-lg sm:rounded-2xl p-3 sm:p-6">
              <div class="flex items-center justify-between mb-2 sm:mb-4">
                <div class="flex items-center gap-1.5 sm:gap-3">
                  <span class="text-xl sm:text-3xl">🕶️</span>
                  <div class="min-w-0 flex-1">
                    <h4 class="font-semibold text-gray-900 text-xs sm:text-base truncate">Poster anonymement</h4>
                    <p class="text-xs sm:text-sm text-gray-600 truncate">Votre identité restera cachée</p>
                  </div>
                </div>
                <label class="relative inline-flex items-center cursor-pointer flex-shrink-0">
                  <input v-model="isAnonymous" type="checkbox" class="sr-only peer">
                  <div class="w-10 h-5 sm:w-14 sm:h-7 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-orange-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 sm:after:h-6 sm:after:w-6 after:transition-all peer-checked:bg-gradient-to-r peer-checked:from-orange-500 peer-checked:to-purple-500"></div>
                </label>
              </div>
        ...
        <div v-if="currentStep === 4" class="space-y-3 sm:space-y-6">
          <div class="text-center">
            <h3 class="text-lg sm:text-2xl font-bold text-gray-900 mb-1 sm:mb-2">Interagissez avec l'équipe</h3>
            <p class="text-gray-600 text-xs sm:text-base">Découvrez les fonctionnalités d'interaction</p>
          </div>
          
          <div class="bg-gray-50 rounded-lg sm:rounded-2xl p-3 sm:p-6">
            <div class="flex items-center gap-2 sm:gap-4 mb-2 sm:mb-4">
              <div class="w-6 h-6 sm:w-10 sm:h-10 bg-gray-200 rounded-full flex items-center justify-center flex-shrink-0">
                <span class="text-xs sm:text-lg">👤</span>
              </div>
              <div class="flex-1 min-w-0">
                <p class="text-xs sm:text-sm text-gray-600">Anonymous • il y a 2h</p>
                <p class="text-gray-900 text-xs sm:text-base truncate">Super journée d'équipe ! 🎉</p>
              </div>
            </div>
            ...
            <div class="flex items-center gap-2 sm:gap-6">
              ...
              >
                <span class="text-xs sm:text-lg">{{ demoReaction ? '❤️' : '🤍' }}</span>
                <span class="text-xs sm:text-sm font-medium">{{ demoReaction ? '1' : '0' }}</span>
              </button>
              
              <button class="flex items-center gap-1 sm:gap-2 px-2 sm:px-4 py-1.5 sm:py-2 bg-gray-100 text-gray-600 rounded-full hover:bg-gray-200 transition">
                <span class="text-xs sm:text-lg">💬</span>
        ...
        <!-- Navigation -->
        <div class="flex justify-between pt-3 sm:pt-6 border-t border-gray-200">
          <button
            v-if="currentStep > 1"
            @click="previousStep"
            class="px-3 sm:px-6 py-1.5 sm:py-3 bg-gray-100 text-gray-700 rounded-md sm:rounded-xl font-medium hover:bg-gray-200 transition text-xs sm:text-base"
          >
            ← Précédent
          </button>
          <div v-else></div>
          ...
          <div class="flex gap-1.5 sm:gap-3">
            <button
              ...
              @click="nextStep"
              :disabled="!canProceed"
              class="px-3 sm:px-6 py-1.5 sm:py-3 bg-gradient-to-r from-orange-500 to-purple-500 text-white rounded-md sm:rounded-xl font-medium hover:shadow-lg transition disabled:opacity-50 disabled:cursor-not-allowed text-xs sm:text-base"
            >
              {{ getNextButtonText() }} →
            </button>
            <button
              v-else
              @click="completeGuide"
              class="px-3 sm:px-6 py-1.5 sm:py-3 bg-gradient-to-r from-green-500 to-blue-500 text-white rounded-md sm:rounded-xl font-medium hover:shadow-lg transition text-xs sm:text-base"
            >
              🎉 Commencer
            </button>
          </div>
